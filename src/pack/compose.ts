...
// Universal lead. Every modern image-gen model has trained on captioned
// stock photography, so they readily render any "ISO 200 / 50mm / Mini LF"
// metadata in the prompt as in-frame caption text. This explicit lead tells
// the model to treat the brief as cinematographic direction, not as a
// caption to overlay. Goes at the start so it weights highest.
const NO_TEXT_LEAD = 'Render as a clean photograph. Do not display any text, captions, watermarks, ISO/lens/camera/aspect labels, location tags, or HUD elements in the image. The brief below is direction for how to capture the scene, not text to overlay. '
...
export function composePrompt(pack: Pack, shot: Shot): string {
  ...
  // to drop.
  const positive = [NO_TEXT_LEAD + shot.action, ...styleBodies].filter(Boolean).join(' ')
